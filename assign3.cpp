...
main( int argc, char** argv ){//get arguments from command line, the name of the simulation text file
  ...
  while(!input_file.eof()){
    ...
    //
     ...
     sumofinputpackets=sumofinputpackets + totalinputlen;
...
//MANIPULATING QUEUES!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  while(currentsum>0){
    ...
    isnull = InputQueues[portnumber].Front();
    cout<<isnull;
    if(isnull!= 0){
      for(int i=0; i< qinputlen[portnumber]; i++){
        datastream = InputQueues[portnumber].Front();
        OutputQueues[datastream].Join(datastream);
        Congestion_Size[datastream]++;
      }
      
    }else{
      cout<<"ERROR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!";//DEBUG WHEN FIXED
      return(0);
    }
    cout<<"Hi";
    portnumber++;
    ...
    {
       OutQueues_current[i] = Congestion_Size[i];
